define(['lodash', 'md5', 'MozContactConnector'], function(_, md5, MozContactConnector){
  'use strict';
  ...
  var ContactHandler = (function ContactHandler() {
  	...
  	function saveContact(vcard, cardid/*, cid*/) {
  		getContact(cardid, function(err, person) {
  			if (err === null) {
    ...
    function setContact(vcard) {
      var cardid;
      //var cid = undefined;

      if (vcard.uid === undefined || !vcard.uid || vcard.uid === '') {
        cardid = md5(vcard.objectUrl + vcard.fn+vcard.n['given-name']+vcard.n['family-name']);
      }
      else {
        cardid = md5(vcard.objectUrl + vcard.uid);
      }
